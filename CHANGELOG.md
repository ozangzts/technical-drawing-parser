...
- Added a prompt rule asking the model to describe a GD&T feature control frame, datum reference, or `+/-` tolerance callout in `warnings` if it ever sees one, instead of forcing it into `dimensions` or silently dropping it. `tolerances` still has no designed schema shape, deliberately, since no sample has shown this content yet and this session repeatedly found that schema/rules designed without a real example needed correcting once one appeared. This rule only closes the blind spot of the model not noticing such content at all; it does not attempt real GD&T extraction.
...
- Ran a "blind" cross-check of the session's prompt/validator changes: a fresh Claude Code session with no memory of this project's decisions, given only the literal `build_vlm_prompt()` output and the rendered page image per sample, told to answer in one shot without reading any other project file, processed all 5 DEICO samples into `outputs_claude_code_blind_test/`. This is closer to how the real Anthropic extractor is actually invoked (no cross-file memory, no iterative self-correction) than the first manual comparison pass was. All 5 samples applied the `drawing_number`/form-stamp distinction and the `schematics`/block-diagram distinction correctly and consistently. It also surfaced a real, previously unspecified gap: it produced one repeated `schematics.parameters` entry per repeating unit (e.g. one "Turns Ratio: 1:1,41" entry per stub) instead of one consolidated entry.
- Added a prompt rule to consolidate a `schematics` parameter value that repeats identically across a diagram's repeating units into one entry instead of one per unit, per the gap found by the blind-session cross-check above.
